...
1. **Perception 병목이 전 지역에서 지배적**
   - Human annotation 결과, 모든 지역에서 86.8%가 Perception 실패
   - 이전 Gemini 자동 분석의 "EU는 Reasoning 병목" 결론은 재검토 필요
   - EU에서도 Visual failure가 63.6%로 가장 높음

2. **India 데이터의 심각한 품질 문제**
   - 35개 실패 문항 중 34개(97.1%)가 라벨링 오류
   - VLM 실패가 아닌 데이터셋 자체의 문제
   - 향후 벤치마크 품질 관리의 중요성 시사

3. **Japan, Korea**: 인식(Perception) 병목이 확실함
   - 문자 체계 복잡성이 성능 저하의 주원인
   - OCR/텍스트 인식 개선 시 큰 성능 향상 예상

4. **Taiwan**: 전반적으로 실패가 적음
   - 번체 중국어가 예상보다 잘 처리됨
   - 샘플 수 적어 통계적 해석 제한적
...
Human annotation 결과 (유효 실패 n=38)에서 시각적 요소의 영향:

```
Visual Elements in Valid Failure Cases (N=38):
──────────────────────────────────────────
Element        | Count | % of failures
──────────────────────────────────────────
Visual Failure |  29   |    76.3%
Parsing Failure|   4   |    10.5%
Domain Issue   |   2   |     5.3%
Other          |   3   |     7.9%
──────────────────────────────────────────
```
...
- **문제 인식 실패**: 이미지 내 텍스트/수식을 정확히 읽지 못함
- **언어 인식 실패**: 비라틴 문자 체계(한글, 일본어, 한자)에서 빈번
- **파싱 실패**: 표 구조, 복잡한 레이아웃에서 정보 추출 실패
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  1. PERCEPTION DOMINATES REASONING (Human Verified)              │
│     • 86.8% of valid failures are perception-related             │
│     • Only 5.3% are pure reasoning/knowledge failures            │
│     • VLMs "can't read", not "can't think"                      │
│                                                                  │
│  2. LABELING ERRORS ARE SIGNIFICANT                              │
│     • 54.8% of annotated failures were labeling errors           │
│     • India: 97.1% labeling error rate (data quality issue)      │
│     • Benchmark quality control is critical                      │
│                                                                  │
│  3. REGIONAL BOTTLENECKS (Valid Failures Only)                  │
│     • All regions: Perception bottleneck dominant (>80%)         │
│     • Japan/Korea: Visual + Parsing failures                     │
│     • EU: Still primarily perception (not reasoning)             │
│     • India: Data quality issue, not VLM failure                 │
│                                                                  │
│  4. MODEL-SPECIFIC GAPS ARE HUGE                                │
│     • Gemini: Strong Asian language support (87% Japan)          │
│     • GPT: Weak Asian language support (26% Japan)               │
│     • Gap of 61.6%p on SAME exam!                               │
│                                                                  │
│  5. SCRIPT COMPLEXITY MATTERS                                   │
│     • Japanese (3 scripts) > Korean > Chinese > Latin            │
│     • Vertical text adds additional difficulty                   │
│     • Math + Non-Latin = Worst combination                       │
│                                                                  │
│  IMPLICATION FOR GOVERNMENT AI:                                  │
│  • All regions: Prioritize visual perception over reasoning      │
│  • Asian countries: OCR + layout recognition enhancement         │
│  • Dataset quality: Rigorous human verification essential        │
│  • Model selection: 3x performance gap possible                  │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
